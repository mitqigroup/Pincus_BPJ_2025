function fig_handle = plot_phi_minima_updown(ds_middle, ds_down,slices, varargin)
...
%         k = k+1;
    ...
    elseif strcmpi(args{k},'UseLocalMinName')
        ...
        % should be an array of strings as above
        k = k+1;
    elseif strcmpi(args{k},'PlotVerticalLines')
        vertical_line_locs = args{k+1};
...
for ii=1:length(slices)
    ...
    if plot_stars
        [x,y] = plot_cases_phi(ps, outputs_analyse, type, 'Color', newcolors(ii), ...
...
if plot_vertical_lines
    for ii=1:length(vertical_line_locs)
        vert_loc = vertical_line_locs(ii);
        plot([x(vert_loc), x(vert_loc)], [-1,pi*2], '--', 'Color', newcolors(ii), ...
