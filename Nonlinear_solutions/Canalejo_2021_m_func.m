function out = ...
    Canalejo_2021_m_func(R, phi, Sigma, kappa, m, d, debug_on)
...
function derivs = hamilton(s, y, const)
    ...
    f(1) = u;
    f(2) = cos(psi);
    f(3) = cos(psi)/r*(sin(psi)/r-u) ...
        + 1/2*sin(psi)/cos(psi)*(sin(psi)^2/r^2-u^2+4*m*sin(psi)/r+2*(Sigma+4*m^2*kappa)/kappa);
    f(4) = sin(psi);
    f(5) = pi*kappa*r*((u+sin(psi)/r)*(u+sin(psi)/r+4*m)+2*(Sigma+2*m^2*kappa)*(1-cos(psi)));
    f(6) = 2*pi*r*(1-cos(psi));
    f(7) = 4*pi*kappa*r*(1/2*(u+sin(psi)/r)-m)^2;

    derivs = f';
end

function [value,isterminal,direction] = myEventFcn(s,y,const, condition)
    psi = y(1);
    r = y(2);
    d = const(6);
...
function [diffs, out] = get_shape(psi_dot_init, constants, condition)
    ...
    init_vals = [phi, r_phi, psi_dot_init, h_phi, 0, 0,0];
    event_func = @(s,y) myEventFcn(s,y,constants, condition);
    options = odeset('Events', event_func, 'RelTol', 1e-9, 'AbsTol',1e-11);
    out = ode45(@(s, y) hamilton(s, y,constants),[0,2*d],init_vals,options);
