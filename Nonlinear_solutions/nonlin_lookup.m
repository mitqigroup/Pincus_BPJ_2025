...
% This is all relative to the local filesystem, must be changed to give
% paths to folders where external functions are stored.
lookup_table = load("/home/gridsan/ipincus/membranes/codes_scripts/" + ...
    "Membrane_scripts/Nonlinear_solutions/gridded_interp.mat");
...
% comment out to run on supercloud, where TaskIDs are provided.
MyTaskID = 0;
NumberOfTasks = 1;
...
R_vals = [0.1];                 % um
sigma_vals = [1e-5];                % surface fraction
%d_vals = sqrt(R_vals.^2./sigma_vals);           % um
% d = 3.1623;
kD_vals = logspace(-2,2,1);                        % picoJ/um^2
% zeta_vals = 1e-5;                  % dimensionless
epsilon_vals = -logspace(-9,-2,1);              % picoJ/um^2
n0_vals = 1;                                    % fraction
kappa_vals = logspace(-9,-5,1);         % picoJ
...
%         assert(alpha_B_init>0.99*alpha_i);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if phi<3.09
            options = optimoptions('fmincon','MaxFunEvals', 1e5, 'MaxIter', ...
                1e5, 'algorithm', 'sqp');   
            ...
            if a>1
                E_bend_B = 2*pi*kappa*(a^2/sqrt(a^2-1)*...
                    (acot(sqrt(a^2-1))+atan((a*tan(phi/2)-1)/sqrt(a^2-1)))...
                    -2*(1-cos(phi)));
            else
                E_bend_B = 2*pi*kappa*(a^2/sqrt(1-a^2)*...
                    (-acoth(sqrt(1-a^2))+atanh((1-a*tan(phi/2))/sqrt(1-a^2)))...
                    -2*(1-cos(phi)));
            end
        ...
        if phi>pi/2&&jj~=1
            ...
            if psi_end>0.1
                % now we just skip
                E_all_nonlinear_self(:,jj,ii) = ...
                    E_all_nonlinear_self(:,jj-1,ii);
                alpha_A_vals_nonlinear_self(jj,ii) = ...
                    alpha_A_vals_nonlinear_self(jj-1,ii);
                alpha_B_vals_nonlinear_self(jj,ii) = ...
                    alpha_B_vals_nonlinear_self(jj-1,ii);
                S_A_vals_nonlinear_self(jj,ii) = ...
                    S_A_vals_nonlinear_self(jj-1,ii);
                S_B_vals_nonlinear_self(jj,ii) = ...
                    S_B_vals_nonlinear_self(jj-1,ii);
                Sigma_vals_nonlinear_self(jj,ii) = ...
                    Sigma_vals_nonlinear_self(jj-1,ii);
        ...
        const = [epsilon, n0, d, R, kD, kappa, alpha_i, N, phi];
        [out,fval,exitflag,output,lam_vals,grad,hessian] = ...
            get_nonlinear_minimum_lookup(const, [alpha_A_init, alpha_B_init], ...
            lookup_table);
        ...
        S_A = 2*pi*R^2*(1-cos(phi));
        S_B = out.y(8,end)*2*pi + pi*((d/2)^2-out.y(2,end)^2) ...
            + d^2*(1-pi/4)+pi*R^2*sin(phi)^2;
...
function [out,fval,exitflag,output,lam_vals,grad,hessian] = ...
    ...
    options = optimoptions('fmincon','MaxFunEvals', 1e5, 'MaxIter', 1e3, ...
        ...
        1e-12, 'StepTolerance', 1e-12, 'Display', 'off');
    [out,fval,exitflag,output,lam_vals,grad,hessian] = ...
        fmincon(@objective,[alpha_A_init, alpha_B_init],...
