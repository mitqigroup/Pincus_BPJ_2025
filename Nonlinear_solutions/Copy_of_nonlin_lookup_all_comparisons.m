...
R_vals = [0.1];                 % um
sigma_vals = pi*(R_vals/0.5)^2;                % surface fraction
%d_vals = sqrt(R_vals.^2./sigma_vals);           % um
% d = 3.1623;
kD_vals = 0.01;                        % picoJ/um^2
kD_base = 0.01;                        % picoJ/um^2
% zeta_vals = 1e-5;                  % dimensionless
epsilon_vals = -6.3096e-06;              % picoJ/um^2
n0_vals = 1;                                    % fraction
kappa_vals = 1.0000e-07;         % picoJ
% kappa_vals = logspace(-21,-15, 60)*1e12;
alpha_i_vals = [0.002];                            % fraction
% phi_vals = flip(deg2rad(linspace(0.01,179.9,5)));
% phi_vals = smoothstep(linspace(0.001,1-0.01,3),2)*pi;
phi_vals = deg2rad(44.9825);
% other constants
N = 3e5;
...
% % R_vals = [0.05,0.5];                 % um
% R_vals = [0.05];                 % um
% % sigma_vals = logspace(-4,-1,24);                % surface fraction
% sigma_vals = 1e-9;                % surface fraction
% %d_vals = sqrt(R_vals.^2./sigma_vals);           % um
% kD_vals = 300/10^12*1e9;                        % picoJ/um^2
% kD_base = 300/10^12*1e9;                        % picoJ/um^2
% % zeta_vals = 0.00051572;                  % dimensionless
% % epsilon_vals = -zeta_vals*kD_base;              % picoJ/um^2
% epsilon_vals = -1e-4;              % picoJ/um^2
% n0_vals = 1;                                    % fraction
% kappa_vals = 1e-19*1e12;         % picoJ
% % kappa_vals = logspace(-21,-15, 60)*1e12;
% alpha_i_vals = 20/3*1e-5;                            % fraction
% phi_vals = flip(deg2rad(linspace(0.01,179.9,50)));
% % other constants
% N = 3e5;
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        const = [epsilon, n0, d, R, kD, kappa, alpha_i, N, phi];
        [out,fval,exitflag,output,lam_vals,grad,hessian] = ...
            get_nonlinear_minimum_lookup(const, [alpha_A_init, alpha_B_init], lookup_table);
        ...
        S_A./(1+alpha_A)+S_B./(1+alpha_B)-d^2/(1+alpha_i);
...
%     smi = lookup_table.smi;
%     pmi = lookup_table.pmi;
%     dmi = lookup_table.dmi;
% %     phi_end_mesh = lookup_table.phi_end;
%     E_bend_bar_mesh = permute(lookup_table.E_bend_bar,[2,3,1]);
%     delA_bar_mesh = permute(lookup_table.delA_bar,[2,3,1]);
%     rend_bar_mesh = permute(lookup_table.rend_bar,[2,3,1]);
...
%         F_Ebend = griddedInterpolant(sgi,pgi,dgi,permute(E_bend_bar,[3,2,1]));
% F_delA = griddedInterpolant(sgi,pgi,dgi,permute(delA_bar,[3,2,1]));
% F_rend = griddedInterpolant(sgi,pgi,dgi,permute(rend_bar,[3,2,1]));
%         E_bend_free = interp3(smi,pmi,dmi,E_bend_bar_mesh,...
%             Sigma*R^2/kappa,phi,d*R, 'linear')*pi*kappa;
%         delA = interp3(smi,pmi,dmi,delA_bar_mesh,...
%             Sigma*R^2/kappa,phi,d*R, 'linear')*R^2;
%         rend = interp3(smi,pmi,dmi,rend_bar_mesh,...
%             Sigma*R^2/kappa,phi,d*R, 'linear')*R;
    end
...
%             rend_s = r(end);
%             rphi_s = r(1);
%             delA_test = (2*pi*out_shape.y(8,end) - pi*(rend_s.^2-rphi_s.^2));
