function out = ...
    free_shape_m_ellipse(a, c, d, phi, kappa, Sigma, m, debug_on)
% solve nonlinear shape equation using hunt and bracket

    warning('off', 'MATLAB:ode45:IntegrationTolNotMet');
    lambda = sqrt(kappa/Sigma);
    sigma = (Sigma+2*kappa*m^2)*a^2/kappa;
    constants = [lambda, a, phi, d, m, c, kappa];
    z = 1-cos(phi);
...
%     f(9) = f(7) + r*2/lambda^2*(1-cos(psi))+4*m^2*r*(1-cos(psi));
    f(9) = r*((f(1)+sin(psi)/r)*(f(1)+sin(psi)/r+4*m)+(2/lambda^2+4*m^2*kappa)*(1-cos(psi)));
...
function [diffs,out] = get_shape(psi_dot_init, constants, condition)
    ...
    p_r_init = (-p_psi_init^2/(4*r_phi)+p_psi_init*(sin(psi_init)/r_phi-2*m)...
        +2*r_phi/lambda^2*(1-cos(phi)))/cos(psi_init);
