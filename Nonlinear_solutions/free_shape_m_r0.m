function out = ...
    free_shape_m_r0(phi, lambda, m, debug_on)
    % solve nonlinear shape equation using hunt and bracket. Expressed in
    % terms of dimensionless variables phi, m = m*r0, lambda = lambda/r0.

    warning('off', 'MATLAB:ode45:IntegrationTolNotMet');
%     lambda = sqrt(kappa/Sigma);
%     sigma = a^2/lambda^2;
    constants = [lambda, a, phi, d, m, c];
    z = 1-cos(phi);
    beta = atan(a/c*tan(phi));
    if beta<0
        beta = beta+pi;
    end
...
%     debug_on = true;
    ...
    if debug_on
        ...
        psi_0 = linspace(-4*sqrt(sigma)/a-2/a,2/a,200);
        ...
        annotation("textbox","String",{sprintf('$\\phi = %0.3g^{\\circ}$', rad2deg(phi)),...
            sprintf('$R = %0.2g$', a), sprintf('$\\bar{\\sigma} = %0.2g$', sigma)});
        % xlim([-20*sqrt(sigma)/R-1,1/R])
        yyaxis left
    end
    
    psi_0_upper = 1/a;
    psi_0_lower = -4*sqrt(sigma)/a-2/a;
            ...
            if ~crossed
                ...
                if ~(isempty(out.xe)||out.ye(1)>0.01)&&(out.ye(2)>d/2)
                    outputs_up = fmincon(@(inp) get_shape(inp, constants, 2), psi_0_iter(counter),...
                                [],[],[],[],-4*sqrt(sigma)/a-2/a,psi_0_iter(counter),[],options);
...
%                 fprintf('alpha = %0.2g\n',alpha)
    ...
    %     psi_0_iter(1) = psi_0(find(correct==0,1,'last'));
        ...
        while (isempty(out.xe)||out.ye(1)>0.01)
            ...
            psi_0_iter(1) = 1/a - alpha*times_searched/2;
            [~,out] = get_shape(psi_0_iter(1), constants, 1);
            times_searched = times_searched + 1;
            if psi_0_iter(1)<(-4*sqrt(sigma)/a-2/a)
...
function out = get_shape(psi_dot_init, constants)
