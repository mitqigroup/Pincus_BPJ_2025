...
if plot_stars
    ...
    if ~(index<1||index>length(x))
        ...
        scatter(x([index_smaller]), y([index_smaller]),300,...
...
if use_newcolours
    v1 = plot(x(index_smaller:end), y(index_smaller:end),markertype,'displayname', displayname, 'Color',colour);
else
    v1 = plot(x(index_smaller:end), y(index_smaller:end),markertype,'displayname', displayname);
end
