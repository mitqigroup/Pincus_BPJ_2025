function output = analyse_phi_curve_updown(ds_middle, ds_down,slice,varargin)
...
%     counter
    ...
    E = E_middle;
    E_removed = E;
    remove_points(abs(E_std)>1e-2) = true;
...
%         yyaxis left
    ...
    % get all minima
    all_global_minima{counter} = get_all_global_mins(ds_middle, xx, ...
        remove_points, jj, global_min_index);
    ...
    elseif any(local_minima)&&global_min_index==...
            find(local_minima,1)&&~phi_pi_is_minima&&length(find(local_minima,1))==1
        curve_type(counter) = 3;
